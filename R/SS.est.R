...
function(total, srates, nh,Nh,stratum, subunit, covars,beta,varbeta, smat=NULL){
 ...
 #  1. total = total number of animals in the observed group
 #  2. srates = sampling rate for the stratum (for each observed animal)
 #  3. nh = number of sampled units in each stratum
 #  4. Nh = number of population units in each stratum
 #  5. Subunit = subunit identifier 
 #  6. covars = matrix of covariates used in the sightability model
 #  7. beta = vector of parameter estimates from logistic model
 #  8. varbeta = var/cov matrix for above parameter estimates

 # thetas = vector of thetas (can override program's thetas, for comparison w/ Steve's code)
...
# Xie = pie= prob. detection = 1/ theta (perhaps this should be inv.logit(exp(xB) instead of 1/theta)
  xie<-1/theta
  
# Point estimate
  Tlr<-sum(total*theta/srates)
...
#   EvarT.D<-sum(((1-xie)/xie^2)*total^2/srates^2) = vd (from Steinhorst and Samuel 1989)
  EvarT.D<-sum(((1-xie)/xie^2)*total^2/srates)  #this version is better, but earlier version to compare w/ Steve's code
...
# Do as much of the matrix multiplication outside of loop as possible
  ...
  for(i in 1:nxdata){
    for(j in i:nxdata){
     ...
     #  smat[i,j]<-smat[j,i]<-exp(-xtot%*%beta-xtot%*%varbeta%*%t(xtot)/2)*(exp(t(xtemp1)%*%varbeta%*%xtemp2)-1)
...
# Determine ajs in SS's estimator of Vm
  ...
  out<-c(Tlr, vartot, varT.D, EvarT.D, Var.mod)

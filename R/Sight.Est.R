...
#   nboot = number of bootstraps if Vm.boot=T
...
# To reduce the chance for errors, change all 
...
#  tempnm2<- as.character(attr(tempnm,"variables"))  
 ...
 #Bootstrap for Cov(theta)? 
  if(Vm.boot==TRUE){
    ...
    nsdat<-nrow(sdat)
    for(i in 1:nboot){
      bdat<-sdat[sample(1:nsdat, replace=TRUE),] # sampel observations with replacement (non-parametric bootstrap)
